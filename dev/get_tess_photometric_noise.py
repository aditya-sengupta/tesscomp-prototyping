...
def get_tess_photometric_noise(sector_num, datapath=utils.TESS_DATAPATH, subpath=None):
    ...
    if datapath is None:
        datapath = os.getcwd()
    if subpath is None:
        subpath = "TESS_noise_S{}.csv".format(sector)
    ...
    if os.path.exists(fullpath):
        ...
        if sum(saved_noise_dataframe.noise != -1.) == len(stlr):
            return saved_noise_dataframe
        else:
            ids_to_skip = set(saved_noise_dataframe[saved_noise_dataframe.noise != -1.].ID.values)
    
    noises = -np.ones(len(stlr))
    try:
